...
@php
    function getActiveNav($currentUrl, $url){
        if ($currentUrl == $url){
            return "flex items-center p-2 0 rounded-lg text-white bg-gray-700 hover:bg-gray-600 group";
        }else{
            return "flex items-center p-2 0 rounded-lg text-white hover:bg-gray-700 group";
        }
    }
@endphp
...
<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
    <div class="h-full px-3 py-4 overflow-y-auto flex flex-col bg-gray-800">
            ...
            @if(auth()->user()->role == 'operator')
                <li>
                    <a href="/checking" wire:navigate class="{{ getActiveNav($currentUrl, '/checking') }}">
                        ...
                        <span class="ms-3">Pemeriksaan QC</span>
                    </a>
                </li>
            @else
                <li>
                ...
                <li>
                    <a href="/items-database" wire:navigate class="{{ getActiveNav($currentUrl, '/items-database') }}">
                        <svg class="w-5 h-5 text-slate-400 transition duration-75 group-hover:text-gray-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        <span class="ms-3">Item Database</span>
                    </a>
                </li>
            @endif
        </ul>
        ...
        <!-- User Info & Logout -->
        <div class="mt-auto text-white text-sm">
            <!-- Divider -->
            ...
            <form method="GET" action="{{ route('logout') }}" class="px-2">
                <button type="submit" class="w-full flex items-center gap-2 p-2 text-sm font-medium text-white bg-red-600 rounded hover:bg-red-700 transition">
